// Q4 — client-side per-thread binding inputs (types/payload only; NO backend, NO signing).
//
// At thread registration the client produces these inputs; the PLATFORM later signs the binding
// (that signature is out of scope for this spike). The `binding` half is what the platform commits
// to and what the settlement attestation references opaquely; the `opening` half (user_id, salt_t)
// stays private until a selective reveal (R11). NEITHER the commitment NOR the opening ever rides
// on the public TxEnvelope — the envelope carries `thread_pubkey` only.

import { newSalt } from "@oursay/public-record/crypto/commitment";
import { threadCommitment } from "./commitment.js";
