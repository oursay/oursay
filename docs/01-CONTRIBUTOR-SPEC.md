...
Area/district membership is **inferred from the user's private geocoded point** (built from the address returned at residency verification), never stored as a district binding on the user. The street-address string is not retained on OurSay — see [`entities/account/future.md`](entities/account/future.md). A single point may resolve to multiple areas at different hierarchy levels simultaneously. Resolution is done dynamically against the platform's boundary registry at query time.

Because boundaries are redrawn over time, district IDs carry a **boundary-year label** (e.g. `edmonton-strathcona-2026`) and are the stable identity of a boundary **revision**. The label is not the lookup key: which geometry applies at an instant is selected by the revision's **`effective_date`** (the boundary registry holds `effective_date`, an optional `drawn_date`, and a year-less riding key — see [`REGION-MODEL.md`](REGION-MODEL.md) and `@oursay/geo`). So a resolution is reproducible against the boundary set **in force on the action's timestamp**; audit/historical integrity comes from the **private geocode point** (or action-time geo snapshot when shipped) + the action's timestamp + the effective-dated boundaries — not from a frozen assignment row. The KYC provider retains the street address; OurSay does not.
