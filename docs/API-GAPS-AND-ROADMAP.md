...
| **`[v1-media-accreditation-bodies]`** | ✅ Catalog table + admin CLI | `auth.accreditation_bodies`; `npm run admin:accreditation-body`. Remaining: public list (if needed) + `recognizedAccreditationBodyIds` on jurisdiction config. |
