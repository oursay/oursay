...
| **`[v1-media-accreditations]`** | ✅ Rows + admin CLI + Media mark wire + act gate | `auth.media_accreditations`; `mediaMark` / `mediaAccredited` on feed/detail/comment DTOs; profile `accreditationBodyIds` (mark inferred); `{ mediaAccredited: true }` gate actor. |
