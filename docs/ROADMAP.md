...
- **Signing gates & prefs** — per-jurisdiction per-action gates (`act` (+ `deny`) / `signMin` / `officialCount`, incl. jurisdiction-residency and official-role gate kinds; the official count is a counting floor after the action, never a participation barrier) replacing the platform-wide vote/signature scheme hard-override; per-account per-action signing preferences (quick/ask/passkey, strongest wins); `signTier` projection on read DTOs.
...
- **Counts** — action-time geo/tier snapshots and signed count snapshots for official totals ([mvp-c4-action-snapshots], [mvp-c13-signed-count-snapshots]).
