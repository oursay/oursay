...
| KYC attestation write seam + tier filter (`@oursay/api`) | Implemented (tests) — pluggable `KycProvider` (stub default; `equifax` reserved → fails fast) behind `KycService.attest`, appending the awarded tier to `public.kyc_attestations` (the same table recovery reads). The public count endpoints resolve a KYC `tier` **set** (current tier ∈ set, AND-combined with geo, k-anonymity floor). A dev-only `POST /v1/dev/kyc/attest` (full session, not registered under `NODE_ENV=production`, hidden from OpenAPI) self-attests a user so QA/tests can place a tier without a raw INSERT. No PII on the row; platform-trust (R26), provider-signed rows (R27) deferred. |
