...
| Finality defaults | Votes and signatures are **changeable by default** (loose platform defaults are intentional); a jurisdiction/entity tightens to final via `allowChange` config (`ab-ca-gov`: `allowChange: false`) — change/revoke only before deadline |
...
| [MediaAccreditation](./account/media-accreditation.md) | Media accreditation | — | `auth.media_accreditations` | [MVP] rows + mark wire; gate actor open |
