...
- **Peer sponsorship and verification waitlist flows.** Pay-for-another-user KYC and the waitlist (`[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §5.6–5.7) are deferred; they only matter under a paid-verify contingency. Launch funding is optional GitHub Sponsors donations (§7.4).
...
| Peer sponsorship / verification waitlist                   | Won't (this launch; paid-verify contingency) | §3; `[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §5.6–5.7                                              |
| Donation soft-ask (GitHub Sponsors) before KYC session     | Must (product intent; UI seam)        | §7.4; `[temp/DONATION-FUNDED-VERIFY-HANDOFF.md](temp/DONATION-FUNDED-VERIFY-HANDOFF.md)`                     |
...
Any registered user can **sign** a petition (for the Alberta jurisdiction, signatures count in the platform count while the signer is residency-verified — sign now, verify later); creating one is per-jurisdiction gated (Alberta: residency-verified). A signer may add an optional comment, hidden when signing anonymously. Signatures are final for the Alberta jurisdiction *by platform config* (`allowChange: false`); the platform default is intentionally loose — changeable/revocable via unified `allowChange` — and other jurisdictions may keep it. For the Alberta jurisdiction, official-role holders may not sign petitions. Petitions are one content type among several — not the product's center of gravity. (`[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §8.2)

### 7.3 Polls and results

Voting eligibility is per-jurisdiction (Alberta jurisdiction: residency-verified Alberta residents, including official-role holders; `oursay-global`: any registered user), optionally anonymous. **Votes are final once cast** for the Alberta jurisdiction *by platform config* (`allowChange: false`); the platform default is intentionally loose — changeable votes and revocable petition signatures via unified `allowChange`. *This is a per-jurisdiction rule. Other jurisdictions may allow changing votes.* Voting runs for a defined period; when it closes, a result will be generated. Vote counts are shown per option, as totals and by tier. A formal derived `result` entity at poll close is a Should for launch (`[API-GAPS-AND-ROADMAP.md](API-GAPS-AND-ROADMAP.md)` `[mvp-c12-poll-results]`); until it lands, closed-poll outcomes are presented honestly as live recomputed counts. (`[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §8.3–8.4)

### 7.4 Identity and verification

Verification is pluggable per jurisdiction; the Alberta launch uses **Didit** for identity verification, with the platform self-signing the address check (proof-of-address in production; ID-only and a platform self-signed address in dev). Equifax and electoral-roll provider tags are future, not launch (`[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §3.3, §5). Tiers map from provider output: identity confirmed → identity verified; identity + address → residency verified (`[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §4.3–4.4, §5.2). Verification is **free to the user**; before opening a Didit session (verify, recover KYC, re-verify) the product soft-asks for optional donations — including recurring — via **GitHub Sponsors** (`[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §5.5; `[02-PUBLIC-EXPLAINER.md](02-PUBLIC-EXPLAINER.md)`). Declining the ask still continues to free verification. Residency verification confirms a real person at a verified address; it is explicitly **not** a determination of electoral eligibility, voter registration, or citizenship, and the platform makes no such claim. An electoral-authority integration (electorally-validated tier) is the designed future path, not a launch requirement (`[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §4.6).

### 7.5 Geography and public counts

District membership is **inferred from the user's private geocoded point** (from the address returned at residency verification) at query time, never stored as a binding on the user (`[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §6.3; `[REGION-MODEL.md](REGION-MODEL.md)`). Street address and legal name stay with the KYC provider. Every aggregate count is filterable by area, verification tier, and date range, and the filters are combinable (`[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §6.4). Narrow geographic or tier buckets are suppressed below a k-anonymity floor to protect privacy (`[API-GAPS-AND-ROADMAP.md](API-GAPS-AND-ROADMAP.md)` Phase C; `[06-PRIVACY-REVIEW.md](06-PRIVACY-REVIEW.md)`). A read-only public API exposes counts, district definitions, results, and audit references, with no PII and no rate limiting that would block legitimate audit or research use (`[01-CONTRIBUTOR-SPEC.md](01-CONTRIBUTOR-SPEC.md)` §7).
...
Internal phase tags (Phase A–E, `[mvp-*]`) are traceability to the MVP prompt playbook <!-- see .agents/MVP-PROMPTS.md --> and `[API-GAPS-AND-ROADMAP.md](API-GAPS-AND-ROADMAP.md)`; they are not part of the stakeholder narrative.
...
4. **Donation UX at launch.** Soft-ask via GitHub Sponsors before Didit sessions is the locked funding model; exact copy, amount suggestions, and ask-vs-skip UX remain product polish. *Default: free verify + optional donate; no payment gateway. Contingency if donations collapse: invasive banners/popups, then pay-per-verification as last resort (`[API-GAPS-AND-ROADMAP.md](API-GAPS-AND-ROADMAP.md)`).*
