...
- **Production `chainId`:** a stable, human-auditable slug — e.g. `ab-ca-gov`. Set via `CHAIN_ID` / `WORKER_CHAIN_IDS` and never reuse for a new genesis (bump a suffix, e.g. `ab-ca-gov-v2`). Immudb database name is `j_` + snake_case of the slug (e.g. `j_ab_ca_gov`).
